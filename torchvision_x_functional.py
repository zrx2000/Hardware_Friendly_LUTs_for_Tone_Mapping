...
def adjust_brightness(img, value=0):
    ...
    if img.dtype in [float, np.float32, np.float64, np.float128]:
    ...
    result = np.clip(img.astype(float)+value, dtype_min, dtype_max).astype(dtype)
...
def adjust_contrast(img, factor):
    ...
    result = np.clip(img.astype(float)*factor, dtype_min, dtype_max).astype(dtype)
