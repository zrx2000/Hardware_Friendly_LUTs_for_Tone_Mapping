...
parser.add_argument("--model_dir", type=str, default="./saved_models/google_hdr_plus", help="directory of pretrained models")		
parser.add_argument("--output_dir", type=str, default="output_hdr_plus", help="directory to save results")  #hdr+
...
criterion_pixelwise = torch.nn.MSELoss()
criterion_pixelwise.cuda()
file = open(os.path.join(opt.image_dir,'test.txt'),'r')
...
for i in range(len(test_input_files)):
    ...
    #hdr+
    img = cv2.imread(os.path.join(opt.image_dir, "google_hdr_plus/input", image_path), -1)
    gt = Image.open(os.path.join(opt.image_dir, "google_hdr_plus/output", image_path.replace(".png", ".jpg")))
    ...
    output_im_path = f'{opt.output_dir}/{image_path.split("/")[-1]}'
